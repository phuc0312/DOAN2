...
if (isset($_POST['update_payment'])) {
    ...
    // $order_id = $_POST['order_id'];
    // // Update the payment status to 'completed' directly in the SQL statement
    // $update_status = $conn->prepare("UPDATE `orders` SET payment_status = 'completed' WHERE id = ?");
...
<body>
    ...
    <section class="show-products" style="padding-top: 0;">
        ...
        <table class="table-category">
            ...
            <tbody>
                <?php
                $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE payment_status = 'wait' ORDER BY placed_on DESC");
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="" method="POST">
                                    ...
                                    <select name="payment_status" class="drop-down">
                                        <option value="wait" <?= ($fetch_orders['payment_status'] === 'wait') ? 'selected' : '' ?>>Chờ xác nhận</option>
                                        <option value="completed" <?= ($fetch_orders['payment_status'] === 'completed') ? 'selected' : '' ?>>Đang chuẩn bị đơn</option>
                            ...
                            <td>
                                <!-- <form action="" method="POST">
                                    <input type="hidden" name="order_id" value="<?= $fetch_orders['id']; ?>">
                                    <div class="flex-btn">
                                        <button type="submit" name="update_payment" class="btn">Xác nhận đơn</button>
                                    </div>
                                </form>------------------ -->
                                <!-- <div class="flex-btn">
                                ...
                                <div class="flex-btn">
                                    <a href="placed_orders_cancel.php?cancel=<?= $fetch_orders['id']; ?>" class="delete-btn">Hủy</a>
                                </div>
                            </td>
                        </tr>
                <?php
